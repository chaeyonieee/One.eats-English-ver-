...
public class AdminOrderControllerImpl implements AdminOrderController {
	...
	public ModelAndView modDeliveryStatus(HttpServletRequest request) {
		...
		try {
			...
			mav = Alert.alertAndRedirect("The modification is complete..", request.getContextPath() +"/admin/order/adminOrderList.do");
		} catch (Exception e) {
			e.printStackTrace();
			mav = Alert.alertAndRedirect("Unable to complete modification.", request.getContextPath() +"/admin/order/adminOrderList.do");
		}
