...
public class SendSMSController {
	...
	public String sendOne(HttpServletRequest request) throws IOException {
		...
		try {
			...
			coolsms.setText("[one.eats] verification code " + randomNumber + " 를 입력하세요.");
...
//    @GetMapping("/get-message-list")
	public MessageListResponse getMessageList() {
		// Search 조건이 있는 경우에 MessagListRequest를 초기화 하여 getMessageList 함수에 파라미터로 넣어서 Search할 수
		// 있습니다!.
		...
		// request.setTo("Search할 수신번호");
		// request.setFrom("Search할 발신번호");

		// 메시지 상태 Search, PENDING은 대기 건, SENDING은 발송 중,COMPLETE는 발송 Complete, FAILED는 발송에 실패한 모든
		// 건입니다.
		/*
		...
		// request.setMessageId("Search할 메시지 ID");

		// Search할 메시지 List
		/*
		 * ArrayList<String> messageIds = new ArrayList<>();
		 * messageIds.add("Search할 메시지 ID"); request.setMessageIds(messageIds);
		 */

		// 조회 할 메시지 유형 Search, 유형에 대한 값은 아래 Content을 참고해주세요!
		// SMS: 단문
		// LMS: 장문
		// MMS: 사진문자
		// ATA: 알림톡
...
//    @PostMapping("/send-many")
	public MultipleDetailMessageSentResponse sendMany() {
		...
		for (int i = 0; i < 3; i++) {
			...
			// 메시지 건건 마다 사용자가 원하는 커스텀 값(특정 Order/Payment 건의 ID를 넣는등)을 map 형태로 기입하여 전송 후 Confirm해볼 수 있습니다!
			/*
