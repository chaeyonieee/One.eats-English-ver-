...
public class KakaoExampleController {
    ...
    public MultipleDetailMessageSentResponse sendMany() {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            Message message = new Message();
            // 발신번호 및 수신번호는 반드시 01012345678 형태로 입력되어야 합니다.
            message.setFrom("Enter the sender's number.");
            message.setTo("Enter the recipient's number.");
            message.setText("If less than 90 characters in English are entered, an SMS-type message will be automatically added." + i);
            message.setKakaoOptions(kakaoOption);
    ...
    public SingleMessageSentResponse sendOneCta() {
        ...
        // 봇 키워드 버튼, 버튼을 클릭하면 버튼 Name으로 수신자가 발신자에게 채팅을 보냅니다.
        KakaoButton kakaoBotKeywordButton = new KakaoButton(
                "테스트 버튼3", KakaoButtonType.BK, null, null, null, null
        );

        // 메시지 전달 버튼, 버튼을 클릭하면 버튼 Name과 친구톡 메시지 Content을 포함하여 수신자가 발신자에게 채팅을 보냅니다.
         ...
         * @see <a href="https://business.kakao.com/info/bizmessage/">상담톡 딜러사 Confirm</a>
         */
